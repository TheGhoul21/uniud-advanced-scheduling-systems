...
function PyodideCodeBlock(code)
  ...
  -- Handle input as table (from YAML) or string (from knitr attributes)
  local input_list = block.attr.input or {}
  if (type(input_list) == "string") then
    -- Parse JSON array string like '["T_naive","P_naive"]' or bare name like 'P_sp'
    local items = {}
    if string.match(input_list, "^%[") then
      for name in string.gmatch(input_list, '"([^"]+)"') do
        table.insert(items, name)
      end
    else
      table.insert(items, input_list)
    end
    input_list = items
  end
  local input = "{" .. table.concat(input_list, ", ") .. "}"
  local ojs_vars = {
    block_id = block_id,
    block_input = input,
  }
...
-- Pre-processing: propagate attributes from knitr .cell divs to child code blocks
-- (knitr strips #| options and places them on the parent div, but the live.lua
-- filter reads them from the code block element)
local function CellDivPreprocess(div)
    ...
    if next(found) then
      div = div:walk({
        CodeBlock = function(cb)
          if cb.classes:includes("pyodide") or cb.classes:includes("webr") then
            for key, val in pairs(found) do
              if not cb.attributes[key] then
                cb.attributes[key] = val
              end
            end
          end
          return cb
